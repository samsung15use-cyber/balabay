...
def main_menu_keyboard(user_id: int = None):
    ...
    # Показываем кнопку админки ТОЛЬКО если user_id совпадает с ADMIN_ID
    if user_id == ADMIN_ID:
...
async def send_broadcast_message(source_message: types.Message, state: FSMContext):
    ...
    # Отправляем статус с кнопкой отмены
    ...
    # Создаём клавиатуру с кнопкой для рассылки, если нужно
    reply_markup = None
    ...
    # Отправляем рассылку
    for idx, user_id in enumerate(users_db.keys()):
        # Проверяем, не отменена ли рассылка
        if broadcast_cancelled:
            await status_msg.edit_text(
                f"<b>❌ РАССЫЛКА ПРЕРВАНА</b>\n\n"
                f"✅ Отправлено: {success}\n"
                f"❌ Ошибок: {fail}\n"
                f"👥 Осталось: {total - idx}\n"
        ...
        try:
            if content.get("has_photo"):
                await bot.send_photo(
                    user_id,
                    content["photo_id"],
                    caption=content["text"] if content["text"] else None,
                    reply_markup=reply_markup,
                    parse_mode="HTML"
                )
                ...
                if content["text"]:
                    ...
                    raise Exception("Нет контента для отправки")
...
async def cancel_broadcast_callback(callback: CallbackQuery):
    ...
    await callback.answer("✅ Рассылка будет отменена после текущего пользователя", show_alert=True)
...
async def cmd_start(message: types.Message):
    ...
    if len(args) > 1:
        ...
        # Проверяем, это ссылка админа?
        if ref_code in admin_refs_db:
            # Увеличиваем счётчик
            admin_refs_db[ref_code]['clicks'] += 1
            save_admin_refs()
        
        # Если не ссылка админа, возможно рефералка пользователя
...
# ========== КОМАНДЫ ДЛЯ РЕФЕРАЛЬНЫХ ССЫЛОК АДМИНА ==========
@dp.message(Command("ref"))
async def cmd_create_ref(message: types.Message):
    if message.from_user.id != ADMIN_ID:
        await message.answer("<b>❌ Команда только для админа</b>", parse_mode="HTML")
        return
    ...
    ref_id = hashlib.md5(str(time.time()).encode()).hexdigest()[:8]
    bot_info = await bot.get_me()
    ref_link = f"https://t.me/{bot_info.username}?start={ref_id}"
    
    # Сохраняем
    admin_refs_db[ref_id] = {
        "id": ref_id,
        "link": ref_link,
        "clicks": 0,
...
async def cmd_ref_stats(message: types.Message):
    ...
    if not admin_refs_db:
        await message.answer("<b>📊 Нет созданных ссылок</b>\n\nИспользуйте /ref для создания", parse_mode="HTML")
        return
    
    text = "<b>📊 СТАТИСТИКА ССЫЛОК</b>\n\n"
    
    for ref_id, ref_data in admin_refs_db.items():
        text += f"🆔 <code>{ref_id}</code>\n"
        text += f"🔗 <code>{ref_data['link']}</code>\n"
        text += f"👥 Переходов: <b>{ref_data['clicks']}</b>\n"
        text += f"📅 Создана: {ref_data['created_at']}\n"
        text += f"─" * 25 + "\n\n"
...
async def cmd_delete_ref(message: types.Message):
    if message.from_user.id != ADMIN_ID:
        await message.answer("<b>❌ Команда только для админа</b>", parse_mode="HTML")
        return
    ...
    args = message.text.split()
    if len(args) < 2:
        await message.answer("<b>❌ Использование:</b> /delref ID_ссылки\n\nID можно посмотреть в /stats", parse_mode="HTML")
        return
    
    ref_id = args[1]
    
    if ref_id in admin_refs_db:
        name = admin_refs_db[ref_id]['id']
        del admin_refs_db[ref_id]
        save_admin_refs()
        await message.answer(f"<b>✅ Ссылка {ref_id} удалена!</b>", parse_mode="HTML")
    else:
        await message.answer(f"<b>❌ Ссылка {ref_id} не найдена</b>", parse_mode="HTML")
...
async def buy_gift(callback: CallbackQuery):
    ...
    await callback.answer()
    await send_with_photo(callback.message.chat.id, "<b>🌸 Главное меню</b>", main_menu_keyboard(user_id), "main_menu", user_id)
...
async def process_promocode(message: types.Message, state: FSMContext):
    ...
    await delete_previous_messages(msg.chat.id, user_id, msg.message_id)
    await state.clear()
    await send_with_photo(message.chat.id, "<b>🌸 Главное меню</b>", main_menu_keyboard(user_id), "main_menu", user_id)
...
async def admin_delete_sponsor(callback: CallbackQuery):
    ...
    if sponsor_id in sponsors_db:
        ...
        await send_with_photo(callback.message.chat.id, "<b>✅ Спонсор удалён</b>", main_menu_keyboard(ADMIN_ID), "main_menu", ADMIN_ID)
    else:
...
async def admin_delete_promocode(callback: CallbackQuery):
    ...
    if promo_id in promocodes_db:
        ...
        await send_with_photo(callback.message.chat.id, "<b>✅ Промокод удалён</b>", main_menu_keyboard(ADMIN_ID), "main_menu", ADMIN_ID)
    else:
...
async def admin_get_broadcast_button_text(message: types.Message, state: FSMContext):
    ...
    # Обработка команд отмены
    if message.text in ["/cancel", "/admin"]:
        await state.clear()
        await send_with_photo(message.chat.id, "<b>👑 Админ панель</b>", admin_keyboard(), "main_menu", ADMIN_ID)
        return
    ...
    msg = await message.answer(
        ...
        "Пример: https://t.me/example или @username",
        parse_mode="HTML"
    )
    await delete_previous_messages(msg.chat.id, ADMIN_ID, msg.message_id)
...
async def back_to_main_menu(callback: CallbackQuery):
    ...
    if not success:
        await callback.answer()
        return
    await send_with_photo(callback.message.chat.id, f"<b>{PREMIUM_FLOWER} Главное меню {PREMIUM_FLOWER}</b>", main_menu_keyboard(user_id), "main_menu", user_id)
    await callback.answer()

# ========== РЕФЕРАЛЬНЫЕ ССЫЛКИ АДМИНА ==========
REF_LINKS_FILE = "admin_refs.json"
...
def save_admin_refs():
    with open(REF_LINKS_FILE, 'w', encoding='utf-8') as f:
        json.dump(admin_refs_db, f, ensure_ascii=False, indent=2)

admin_refs_db = load_admin_refs()
